...
from utils.loss import ComputeLoss,LevelAttention_loss
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        model = Model(opt.cfg or ckpt['model'].yaml,input_mode = opt.input_mode,ch_steam=opt.ch_steam, ch=opt.ch, nc=nc, anchors=hyp.get('anchors'),config=None,sr=opt.super,att=opt.attention,sr_att=opt.super_attention,factor=down_factor).to(device)  # create
        ...
        model = Model(opt.cfg, input_mode = opt.input_mode ,ch_steam=opt.ch_steam,ch=opt.ch, nc=nc, anchors=hyp.get('anchors'),config=None,sr=opt.super,att=opt.attention,sr_att=opt.super_attention,factor=down_factor).to(device)  # create
    with torch_distributed_zero_first(rank):
    ...
    if not opt.data.endswith('SRvedai.yaml'):
        dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            ...
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '),hr_input=opt.hr_input)
    else:
    ...
    if rank in [-1, 0]:
        if not opt.data.endswith('SRvedai.yaml'):
            testloader = create_dataloader(test_path, imgsz_test, batch_size, gs, opt,  # testloader
                                       ...
                                       prefix=colorstr('val: '),hr_input=opt.hr_input)[0]
        else:
        ...
        for i, (imgs, irs, targets, paths, _) in pbar:  # batch zjq  -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if opt.super and not opt.attention and not opt.super_attention:
                    pred,output_sr = model(imgs,irs,opt.input_mode)  # forward #zjq
                elif (opt.super and opt.attention) or opt.super_attention:
                    pred,output_sr, attention_mask = model(imgs,irs,opt.input_mode)
                elif not opt.super and not opt.super_attention and opt.attention:
                    pred, attention_mask = model(imgs,irs,opt.input_mode)
                else:
                    pred = model(imgs,irs,opt.input_mode)
                # t1 = time.time()
                # print(t1-t0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str,default='models/SRyolo_noFocus.yaml', help='model.yaml path')
    parser.add_argument('--super', default=True,action='store_true', help='super resolution')
    parser.add_argument('--super_attention',default=False,action='store_true', help='super resolution with attention in it')
    parser.add_argument('--attention', default=False,action='store_true', help='attention')
    parser.add_argument('--cal_att_loss',default=False, action='store_true', help='attention')
    ...
    parser.add_argument('--ch', type=int,default=4,help = '3 4 16 midfusion1:64 midfusion2,3:128 midfusion4:256') 
