...
class Model(nn.Module):
    export = False  # onnx export
    def __init__(self, cfg='yolov5s.yaml',input_mode='RGB',ch_steam=3, ch=3, nc=None, anchors=None,config=None,sr=False,att=False,sr_att=False,factor=2):  # model, input channels, number of classes
        ...
        if self.sr == True:
            ...
            self.model_up = DeepLab(ch,self.yaml['c1'],self.yaml['c2'],factor=factor) #'if the size is m:192,768 l:256,1024 x:320 1280
            self.l1=self.yaml['l1']
            self.l2=self.yaml['l2']
        if self.sr_att == True:
            from models.deeplab_attention import DeepLab
            self.model_up = DeepLab(ch,self.yaml['c1'],self.yaml['c2'],factor=factor) #'if the size is m:192,768 l:256,1024
            self.l1=self.yaml['l1']
            self.l2=self.yaml['l2']
